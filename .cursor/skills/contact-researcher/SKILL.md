...
description: >-
  Given a company name, finds real, sourced contact signal (recruiter, HR,
  engineering-blog byline, GitHub org member, LinkedIn search-snippet hit) using
  this repo's enrich.py. Never fabricates a plausible-sounding contact — reports
  "nothing found" honestly when that's the real outcome. Invoked by
  promote-dossier at Step 3 (Commit); can also be used standalone for one company.
---

# contact-researcher

Research **one company's** real, public contact signal for the internship-research-loop pipeline. This is the exploratory step in an otherwise deterministic pipeline (see `core/filter.py`, `core/relevance.py`, `core/classify.py` — all zero-LLM, keyword-based) — that is exactly why this step stays a judgment-driven skill instead of a script. The only job here is to look, and to say precisely what was found and where it came from.

If the `contact-researcher` Task subagent type is available in this session, prefer launching it directly — it runs this same procedure in an isolated context. This skill exists so the procedure is followable inline when that subagent type isn't available, or when the user wants the steps run in the main conversation.
...
This repo's `enrich.py` (repo root) already implements every legitimate search surface this pipeline is allowed to use. Read it first if you haven't — don't reimplement its logic from scratch. Reuse its functions directly via a short inline script, run with the `Shell` tool:
...
If this skill was invoked by `promote-dossier` (or another skill), return this as your final message in exactly this structure with no conversational filler before or after it — it's consumed programmatically by the caller, not narrated to a human directly.
